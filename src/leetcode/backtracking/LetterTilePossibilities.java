...
public class LetterTilePossibilities {
	...
	private static void dfs(String tiles, int startPoint, HashSet<String> results, StringBuilder tempResult,
			HashMap<Character, Integer> checker) {
		if (startPoint >= tiles.length())
			return;
	...
	private static boolean validate(HashSet<String> results, String input, HashMap<Character, Integer> checker) {
		...
		for (Map.Entry<Character, Integer> entry : hMap.entrySet()) {
			if (checker.get(entry.getKey()) < entry.getValue())
				return false;
		}
