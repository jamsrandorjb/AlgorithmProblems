...
public class LetterCasePermutation {
	...
	public static void letterCasePermutationRecursice(List<String> casePemutations, char[] chars, int start) {
		...
		for (int i = start; i < chars.length; i++) {
			if (!(chars[i] >= 'a' && chars[i] <= 'z' || chars[i] >= 'A' && chars[i] <= 'Z')) continue;
	...
	public static void switchLetterCase(char[] chars, int index) {
	    char c = chars[index];
	    if (c >= 'a' && c <= 'z') chars[index] -= 32;
	    else if (c >= 'A' && c <= 'Z') chars[index] += 32;
	}           
